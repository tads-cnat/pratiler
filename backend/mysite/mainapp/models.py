...
class Leitor(AbstractUser):
    ...
    seguidores = models.ManyToManyField(
        "self", symmetrical=False, blank=True, related_name="seguidores_de"
    )
    seguindo = models.ManyToManyField(
        "self", symmetrical=False, blank=True, related_name="seguidos_por"
    )
...
class Postagem(models.Model):
    ...
    pagina_inicial = models.IntegerField(
        default=0
    )  # Página inicial do livro que o leitor estava quando fez a postagem
...
class Resenha(models.Model):
    ...
    def __str__(self):
        return (
            f"{self.titulo}"  # apaguei - {self.leitor.id_username} porque nao tem mais
        )
...
class Curtida(models.Model):
    ...
    def __str__(self):
        return (
            ...
            + " curtiu postagem de "
            + self.postagem.leitor.user.username
        )
