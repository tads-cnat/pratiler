...
class GerenciarEstanteTestCase(TestCase):
    def test_adicionar_livro_na_estante_com_sucesso(self):
        self.leitor_add = Leitor.objects.create_user(
            nome="Paulo Ricardo",
            username="Maria3",
            email="Marianita@email.com",
            password="pastel123",
        )
        ...
        self.interacao_add = Interacao.objects.create(
            leitor=self.leitor_add, livro=self.livro_add, status="LN", pg_atual=0
        )
    ...
    def test_livros_lendo(self):
        leitor = Leitor.objects.create_user(
            nome="João Roberto",
            username="JoaoR",
            email="joaorobert0@gmail.com",
            password="joao123",
        )
        ...
        for livro in [l1, l2, l3]:
            Interacao.objects.create(
                leitor=leitor, livro=livro, status="LN", pg_atual=0
            )

        self.assertEqual(
            Interacao.objects.filter(leitor=leitor, status="LN").count(), 3
        )

    def test_livros_quero_ler(self):
        leitor = Leitor.objects.create_user(
            nome="Maria Joaquina",
            username="marijoaquina",
        ...
        for livro in [l1, l2]:
            Interacao.objects.create(
                leitor=leitor, livro=livro, status="QL", pg_atual=0
            )

        self.assertEqual(
            Interacao.objects.filter(leitor=leitor, status="QL").count(), 2
        )
    ...
    def test_livros_lido(self):
        leitor = Leitor.objects.create_user(
            nome="Felipe Ale",
            username="lipeAlves",
            email="lipeeeeeee@gmail.com",
            password="lipe-é-um-bom-gerente",
        )
        ...
        for livro in [l1, l2, l3]:
            Interacao.objects.create(
                leitor=leitor, livro=livro, status="LD", pg_atual=0
            )

        self.assertEqual(
            Interacao.objects.filter(leitor=leitor, status="LD").count(), 3
        )
