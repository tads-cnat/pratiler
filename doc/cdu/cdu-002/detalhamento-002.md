# CDU002. Gerenciar feed 

- **Ator principal**: Leitor
- **Atores secundários**: ...	 
- **Resumo**: O leitor poderá gerenciar seu feed, navegando e gerenciando e explorando as funcionalidades das páginas de "Desafios", "Minha estante", "Livros populares" e "Seguindo"
- **Pré-condição**: Estar autenticado no sistema
- **Pós-Condição**: ...

## Fluxo Principal
&emsp;&emsp;Após se autenticar, o leitor verá a página de "Desafios", onde poderá consultar os desafios inseridos pelo moderador do sistema, nesta consulta pode-se ver o nome, descrição, e quantidade de participantes do desafio.<br>
&emsp;&emsp;Nestes desafios, o leitor tem a opção de participar deles na função de 'participar', assim tendo a missão de completá-lo.

## Fluxo Alternativo I - Página "Minha estante"
&emsp;&emsp;O leitor após se autenticar, pode navegar para a página "Minha estante", onde terão os livros que o leitor adicionou. Lá terão as informações dos livros como capa, descrição, e número de páginas lidas até o momento, que poderá ser atualizado de acordo com o progresso do leitor, além de poder adicionar mais livros.<br>
&emsp;&emsp;A página está dividida em 3 listagens de livros, "lidos", onde somente mostra os livros que o leitor já completou, "lendo", que é a listagem principal descrita no primeiro parágrafo, e "desejo ler", onde lista os livros que o leitor marcou para ler depois, podendo adicionar a listagem "lendo" futuramente.

## Fluxo Alternativo II - Página "Livros populares"
&emsp;&emsp;O leitor após se autenticar, pode navegar para a página "Livros populares", onde terão os livros mais populares atualmente rankeados por números ordinais(1º, 2º, ...). Em consulta, poderão ser vistos a capa, o título, a descrição e a quantidade de pessoas que leram este livro.<br>
&emsp;&emsp;Nesta página, também há a funcionalidade de adicionar a lista de livros "desejo ler", caso seja de desejo do leitor, sendo uma segunda opção para adicionar livros às suas listas.

## Fluxo Alternativo III - Página "Seguindo"
&emsp;&emsp;O leitor após se autenticar, pode navegar para a página "Seguindo", onde poderá ver com mais facilidade as postagens de seus seguidores, postagens essas que tem como informações, foto e nome do usuário que postou, o nome do livro tema do *post*, páginas lidas no momento da postagem, e o comentário postado.<br>
&emsp;&emsp;Nestas postagens, os leitores que visualizarem, podem curtir o comentário, ou comentar sobre o comentário, servindo como uma resposta ao comentário, como um meio de socialização.

## Fluxo Alternativo IV - Página "Desafios"
&emsp;&emsp;O leitor terá acesso aos desafios registrados no sistema que ele poderá participar. Cada desafio possuirá uma descrição, título e quantidade de pessoas que estão participando. Clicando em "Participar do desafio", o leitor se tornará um dos participantes.

## Fluxo Alternativo V - Página "Perfil"

## Diagrama de Interação (Sequência ou Comunicação)

> Substituir pela imagem correspondente...

## Diagrama de Classes de Projeto

> Substituir pela imagem contendo as classes (modelo, visão e templates) que implementam o respectivo CDU...
